...
fn run(path: &Path) -> Result<(), String> {
    ...
    let record: CaptureRecord = serde_json::from_str(&data).map_err(
        |err| {
            format!(
    ...
    let bytes = record
        ...
        .map_err(|err| match err {
            CaptureBodyDecodeError::MissingBody => format!(
                "capture file {} has no body field",
                path.display()
            ),
            CaptureBodyDecodeError::UnsupportedEncoding { encoding } => {
                format!(
                    "capture body encoding \"{encoding}\" is not supported; expected \"base64\"",
                )
            }
            CaptureBodyDecodeError::InvalidBase64(err) => format!(
                "capture body data is not valid base64: {err}"
            ),
        })?;
    ...
    stdout
        .write_all(&bytes)
        .map_err(|err| {
            format!("failed to write body to stdout: {err}")
        })?;
    stdout
        .flush()
        .map_err(|err| {
